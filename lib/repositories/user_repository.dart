...
class UserRepository {
  ...
  Future<User> createUser(String email, String name, String surname) async {
    ...
    final newDoc = await firestore.collection(collectionName).doc(newDocRef.path).get();

    return User.fromJson({'id': newDoc.id, ...?newDoc.data()});
  }
  
  Future<User> findUserByEmail(String email) async {
    final querySnapShot = await firestore.collection(collectionName).limit(1).where('email', isEqualTo: email).get();
